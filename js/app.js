...
function showToast(m) { let t = $('toast'); t.textContent = m; t.classList.add('on'); setTimeout(() => t.classList.remove('on'), 2500); }

function showNotification(notif) {
    if ($('ppage')?.classList.contains('open') || $('watchPartyModal')?.classList.contains('active')) { loadNotifications(); return; }
...
$('btnLogin').addEventListener('click', async ()=>{ 
    ...
    if(!email||!pass) { if(err){err.textContent='يرجى إدخال البريد وكلمة المرور';err.classList.add('show');} return; } 
    ...
    try { 
        ...
        if(error) { if(err){err.textContent=error.message;err.classList.add('show');} btn.innerHTML=originalText; btn.disabled=false; } 
        else { 
            ...
            const greetingText = $('greetingText'); if(greetingText) greetingText.textContent = '👋 أهلاً بك ' + displayName + '!'; 
            const greetingSplash = $('greetingSplash'); if(greetingSplash) greetingSplash.classList.add('active'); 
    ...
    } catch(e) { if(err){err.textContent=e.message;err.classList.add('show');} btn.innerHTML=originalText; btn.disabled=false; } 
});
...
$('btnRegister').addEventListener('click', async ()=>{ 
    ...
    err?.classList.remove('show'); 
    if(!email||!pass||!confirm) { if(err){err.textContent='يرجى ملء جميع الحقول';err.classList.add('show');} return; } 
    if(pass!==confirm) { if(err){err.textContent='كلمتا المرور غير متطابقتين';err.classList.add('show');} return; } 
    try { 
        const { error } = await authGateway('register', { email, password: pass, displayName: name }); 
        if(error) { if(err){err.textContent=error.message;err.classList.add('show');} } 
        else { alert('تم إنشاء الحساب بنجاح! سجل دخول الآن.'); $('tabLoginModal')?.click(); if($('loginEmail')) $('loginEmail').value = email; } 
...
$('btnForgotPass')?.addEventListener('click', async () => {
    ...
    if (email) {
        const { error } = await dbClient.auth.resetPasswordForEmail(email, { redirectTo: window.location.origin });
        if (error) alert('فشل إرسال رابط إعادة التعيين');
        else alert('تم إرسال الرابط إلى بريدك');
    }
});
...
async function logout() { 
    ...
    if($('userBtn')) $('userBtn').style.display='flex'; if($('userInfo')) $('userInfo').style.display='none'; if($('notifBtn')) $('notifBtn').style.display='none';
    showAllSections(); 
...
function updateUIAfterLogin() { 
    ...
    if($('userEmoji')) $('userEmoji').innerHTML = avatarToIcon(avatarEmoji);
    if($('userName')) $('userName').textContent = currentUser.user_metadata?.display_name || currentUser.email; 
}

function toggleUserDropdown() { const dd = $('userDropdown'); if (dd) dd.classList.toggle('show'); }
...
function openRecommendModal() {
    ...
    if(list) {
        list.innerHTML = friends.map(f => `<div class="friend-item"><div class="finfo"><div class="favatar">${avatarToIcon(f.avatar || '👤')}</div><span>${f.display_name}</span></div><button class="login-btn" style="width:auto;padding:6px 14px;font-size:11px;" onclick="sendRecommendation('${f.id}')">إرسال 💌</button></div>`).join('');
    }
}
async function sendRecommendation(friendId) {
    ...
    try {
        await dbClient.from('notifications').insert({ user_id: friendId, sender_id: currentUser.id, type: 'recommendation', message: msg, data: safeData });
...
// ========== FRIENDS ==========
function openFriends() { closeUserDropdown(); if (!currentUser) { showLogin(); return; } toggleModal('friendsModal', true); loadFriendRequests(); loadFriends(); }
async function searchFriend() {
    ...
    if (data && data.length > 0) {
        resultDiv.innerHTML = data.filter(u => u.id !== currentUser.id).map(u => { const isFriend = friends.some(f => f.id === u.id); let btnHtml = isFriend ? `<span style="font-size:10px;color:var(--green);background:var(--bg4);padding:4px 8px;border-radius:10px;">صديق لديك</span>` : `<button class="login-btn" style="width:auto;padding:6px 14px;font-size:11px;" onclick="sendFriendRequest('${u.id}')">إضافة ➕</button>`; return `<div class="friend-item"><div class="finfo"><div class="favatar">${avatarToIcon(u.avatar || '👤')}</div><span>${u.display_name}</span></div>${btnHtml}</div>`; }).join('');
...
function renderFriendRequestsUI() {
    const list = $('friendRequestsList'); if (!list) return;
    if (friendRequests.length === 0) { list.innerHTML = '<p style="color:var(--t3);">لا توجد طلبات</p>'; return; }
    list.innerHTML = friendRequests.map(req => `<div class="request-item"><div class="finfo"><div class="favatar">${avatarToIcon(req.sender?.avatar || '👤')}</div><span>${req.sender?.display_name || 'مستخدم'}</span></div><div style="display:flex;gap:6px;"><button class="login-btn" style="width:auto;padding:4px 8px;font-size:10px;background:var(--green);" onclick="acceptFriendRequest('${req.id}', '${req.sender_id}')">✅</button><button class="login-btn" style="width:auto;padding:4px 8px;font-size:10px;background:#ef4444;" onclick="rejectFriendRequest('${req.id}', '${req.sender_id}')">❌</button></div></div>`).join('');
}
...
async function loadFriends() {
    ...
    if (!friendRows || friendRows.length === 0) { friends = []; } else {
        ...
        const { data: profiles } = await dbClient.from('profiles').select('id, display_name, avatar').in('id', fIds);
        friends = profiles || [];
    }
    renderFriendsListUI();
}
function renderFriendsListUI() {
    ...
    list.innerHTML = friends.map(f => `<div class="friend-item"><div class="finfo"><div class="favatar">${avatarToIcon(f.avatar || '👤')}</div><span>${f.display_name}</span></div><div style="display:flex;gap:6px;"><button class="login-btn" style="width:auto;padding:4px 10px;font-size:10px;" onclick="openDM('${f.id}','${f.display_name}')">💬</button><button class="login-btn" style="width:auto;padding:4px 10px;font-size:10px;background:transparent;color:#ef4444;border:1px solid #ef4444" onclick="removeFriend('${f.id}')">🗑️</button></div></div>`).join('');
}
...
// ========== SETTINGS ==========
function openSettings() { closeUserDropdown(); if (!currentUser) return; if($('setDisplayName')) $('setDisplayName').value = currentUser.user_metadata?.display_name || ''; if($('setNewPass')) $('setNewPass').value = ''; const currentAvatar = currentUser.user_metadata?.avatar || '😊'; const avatarSelection = $('avatarSelection'); if(avatarSelection) avatarSelection.innerHTML = PRESET_AVATARS.map(a => `<div class="favatar ${a===currentAvatar?'selected':''}" style="cursor:pointer; border:${a===currentAvatar?'2px solid var(--gold)':'none'}" onclick="selectAvatar(this, '${a}')">${a}</div>`).join(''); if($('selectedAvatarInput')) $('selectedAvatarInput').value = currentAvatar; toggleModal('settingsModal', true); }
function selectAvatar(el, avatar) { document.querySelectorAll('#avatarSelection .favatar').forEach(d => d.style.border = 'none'); el.style.border = '2px solid var(--gold)'; if($('selectedAvatarInput')) $('selectedAvatarInput').value = avatar; }
async function saveSettings() { 
    ...
    try { 
        ...
        renderFriendsListUI(); 
        renderFriendRequestsUI(); 
        if (wpChannel && wpIsHost) updateUsersListBroadcast(); 
        closeSettings(); 
        closeUserDropdown(); 
        showToast('✅ تم الحفظ'); 
...
function openAdmin() { if (!isAdmin) return; toggleModal('adminModal', true); switchAdminTab('stats', document.querySelector('#adminModal .admin-tab')); }
async function switchAdminTab(tab, btn) { 
    ...
    document.querySelectorAll('#adminModal .admin-section').forEach(s => s.classList.remove('on')); if($('admin-'+tab)) $('admin-'+tab).classList.add('on'); 
...
async function init(){
    ...
    initGenres(); 
    await loadHero(); 
    try{ let m=await fetchMov(1); appendCards('g-mov',m); if($('b-mov')) $('b-mov').style.display='block'; }catch(e){} 
    try{ let t=await fetchTV(1); appendCards('g-tv',t); if($('b-tv')) $('b-tv').style.display='block'; }catch(e){} 
}
...
function switchTab(tab,el){ 
    ...
    if(tab==='fav'){ if($('sec-fav')) $('sec-fav').style.display='block'; renderFavorites(); } 
    else if(tab==='hist'){ if($('sec-hist')) $('sec-hist').style.display='block'; renderHistory(); } 
    else { 
        if($('sec-all')) $('sec-all').style.display='block'; let trend=$('g-trend')?.parentElement, mov=$('g-mov')?.parentElement, tv=$('g-tv')?.parentElement; 
...
async function openDetail(item){
    ...
    if($('d-back')) $('d-back').src=''; if($('d-poster')) $('d-poster').src=''; if($('dep')) $('dep').style.display='none'; if($('d-seasons')) $('d-seasons').innerHTML=''; if($('d-eps')) $('d-eps').innerHTML=''; if($('d-meta'))$('d-meta').innerHTML=''; if($('d-cast-wrap'))$('d-cast-wrap').style.display='none'; if($('d-cast'))$('d-cast').innerHTML='';
    const dov = $('dov'); if(dov) dov.classList.add('open'); document.body.style.overflow='hidden';
    if($('d-title')) $('d-title').textContent=title; if($('d-ov')) $('d-ov').textContent=item.overview||'جاري التحميل...';
    if(item.backdrop_path && $('d-back')) $('d-back').src=`${IMG}w1280${item.backdrop_path}`; if(item.poster_path && $('d-poster')) $('d-poster').src=`${IMG}w500${item.poster_path}`;
    if($('d-tags')) $('d-tags').innerHTML=`<span class="dtag gold">⭐ ${rat}</span><span class="dtag">${year}</span><span class="dtag">${curType==='movie'?'🎬 فيلم':'📺 مسلسل'}</span>`;
    ...
    try{
        ...
        curItem={...item,...det};
        if(det.backdrop_path && $('d-back')) $('d-back').src = `${IMG}w1280${det.backdrop_path}`; if(det.poster_path && $('d-poster')) $('d-poster').src = `${IMG}w500${det.poster_path}`;
        if($('d-ov')) $('d-ov').textContent = det.overview || item.overview || 'لا يوجد وصف';
        let genres=(det.genres||[]).map(g=>`<span class="dtag">${g.name}</span>`).join(''); if($('d-tags')) $('d-tags').innerHTML+=genres;
        let trailer = det.videos?.results?.find(v => v.type === "Trailer" && v.site === "YouTube"); if (trailer) { trailerKey = trailer.key; if($('trailerWatchBtn')) $('trailerWatchBtn').classList.add('show'); } else { trailerKey = null; if($('trailerWatchBtn')) $('trailerWatchBtn').classList.remove('show'); }
        ...
        if(det.credits&&det.credits.cast){ let cast=det.credits.cast.slice(0,8); if($('d-cast')) $('d-cast').innerHTML=cast.map(c=>`<div class="dcast-item">${c.profile_path?`<img src="${IMG}w185${c.profile_path}">`:`<div style="width:48px;height:48px;border-radius:50%;background:var(--bg4);display:flex;align-items:center;justify-content:center;font-size:16px">👤</div>`}<span>${c.name}</span></div>`).join(''); if($('d-cast-wrap')) $('d-cast-wrap').style.display='block'; }
        if(curType==='tv'&&det.seasons){ let seasons=det.seasons.filter(s=>s.season_number>0); if(seasons.length){ if($('dep')) $('dep').style.display='block'; if($('d-seasons')) $('d-seasons').innerHTML=seasons.map((s,i)=>`<button class="sbtn ${i===0?'on':''}" onclick="selSeason(${s.season_number},${s.episode_count},this)">${s.name||'الموسم '+s.season_number}</button>`).join(''); renderEps(1,seasons[0].episode_count); } }
    }catch(e){}
}
...
function renderEps(season,total){ let max=Math.min(total,100); if($('d-eps')) $('d-eps').innerHTML=Array.from({length:max},(_,i)=>`<button class="epbtn ${i===0?'on':''}" onclick="selEp(${i+1},this)">ح${i+1}</button>`).join(''); }
function selEp(n,btn){ curEp=n; document.querySelectorAll('#d-eps .epbtn').forEach(b=>b.classList.remove('on')); btn.classList.add('on'); }
function closeDetail(){ const dov = $('dov'); if(dov) dov.classList.remove('open'); document.body.style.overflow=''; }
function playTrailer() { if (trailerKey) { if($('trailerFrame')) $('trailerFrame').src = `https://www.youtube.com/embed/${trailerKey}?autoplay=1`; toggleModal('trailerModal', true); } }
function closeTrailer() { if($('trailerFrame')) $('trailerFrame').src = ''; toggleModal('trailerModal', false); }
async function openPlayerFromDetail(){ 
    if (!currentUser) { showLogin(); return; } closeDetail(); 
    let title=curItem.title||curItem.name||''; if($('ptitle')) $('ptitle').textContent=curType==='tv'?`${title} — م${curSeason} ح${curEp}`:title; 
    const ppage = $('ppage'); if(ppage) ppage.classList.add('open'); document.body.style.overflow='hidden'; document.querySelectorAll('.psrc').forEach((b,i)=>b.classList.toggle('on',i===0)); curSrc=0; 
    if(curType==='tv'&&curItem.seasons&&curItem.seasons.length){ if($('pep')) $('pep').style.display='block'; buildPepRow(); } else { if($('pep')) $('pep').style.display='none'; } 
...
function pepSeason(n,ec,btn){ curSeason=n; curEp=1; buildPepRow(); loadFrame(0); if($('ptitle')) $('ptitle').textContent=`${curItem.title||curItem.name} — م${curSeason} ح${curEp}`; }
function pepEp(n,btn){ curEp=n; document.querySelectorAll('#pep-row .pep-epbtn').forEach(b=>b.classList.remove('on')); if(btn)btn.classList.add('on'); loadFrame(0); if($('ptitle')) $('ptitle').textContent=`${curItem.title||curItem.name} — م${curSeason} ح${curEp}`; }
function loadFrame(startTime = 0){ 
    if($('pload')) $('pload').style.display='flex'; 
    const pframe = $('pframe'); if(pframe) pframe.src=''; 
    setTimeout(()=>{ 
        ...
        pframe.onload = () => { if (pframe.hasAttribute('sandbox')) pframe.removeAttribute('sandbox'); if($('pload')) $('pload').style.display='none'; startProgressTimer(); };
        setTimeout(()=>{ if($('pload')) $('pload').style.display='none'; },6000); 
    },150); 
}
...
function closePlayer(){ stopProgressTimer(); const ppage = $('ppage'); if(ppage) ppage.classList.remove('open'); const pframe = $('pframe'); if(pframe) pframe.src=''; document.body.style.overflow=''; showAllSections(); }

// ========== SEARCH ==========
function performSearch(){ 
    let q=$('sinput')?.value.trim(); if(searchAbortController)searchAbortController.abort(); 
    if(q?.length<2){ if(originalMainHTML && $('main')) $('main').innerHTML=originalMainHTML; showAllSections(); return; } 
    searchAbortController=new AbortController(); let signal=searchAbortController.signal; 
    hideAllSections(); if($('search-results')) $('search-results').style.display='block'; if($('g-search')) $('g-search').innerHTML='<div style="grid-column:1/-1;text-align:center;padding:20px;color:var(--t2)">جاري البحث...</div>'; 
    fetchSearch(q,1,signal).then(res=>{ if(signal.aborted)return; if($('g-search')) $('g-search').innerHTML=''; if(!res.length) { if($('g-search')) $('g-search').innerHTML='<div style="grid-column:1/-1;text-align:center;padding:20px;color:var(--t2)">لا توجد نتائج</div>'; } else res.forEach(item=>{ if(item.media_type!=='person'&&(item.poster_path||item.backdrop_path)) if($('g-search')) $('g-search').appendChild(mkCard(item)); }); }).catch(()=>{}); 
}
if($('sinput')) $('sinput').addEventListener('input', () => { clearTimeout(searchTimeout); searchTimeout = setTimeout(performSearch, 600); });
...
function initBroadcast(asHost) {
    ...
    wpChannel.on('broadcast', { event: 'member_join' }, ({ payload }) => { if (!wpMembers.some(m => m.userId === payload.userId)) { wpMembers.push(payload); updateUsersListBroadcast(); } });
    wpChannel.on('broadcast', { event: 'member_leave' }, ({ payload }) => { wpMembers = wpMembers.filter(m => m.userId !== payload.userId); updateUsersListBroadcast(); });
    wpChannel.subscribe(async (status) => { if (status === 'SUBSCRIBED' && asHost) { wpChannel.send({ type: 'broadcast', event: 'member_join', payload: { userId: currentUser.id, displayName: currentUser.user_metadata?.display_name || 'المضيف', avatar: currentUser.user_metadata?.avatar || '👑' } }); } });
}
function sendSyncCommand(command, time) { if (!wpIsHost || !wpChannel) return; wpChannel.send({ type: 'broadcast', event: 'sync', payload: { command, time } }); }
function sendChatMessageBroadcast(msg) { if (!wpChannel) return; const displayName = currentUser?.user_metadata?.display_name || 'مجهول'; wpChannel.send({ type: 'broadcast', event: 'chat', payload: { displayName, message: msg } }); }
...
async function createWatchParty() { if (!currentUser) { showLogin(); return; } closeDetail(); wpRoomCode = Math.random().toString(36).substring(2,8).toUpperCase(); wpIsHost = true; const token = JSON.parse(localStorage.getItem('shush_session')).access_token; try { await gatewayRequest('rooms','POST',{ host_id:currentUser.id, movie_id:String(curItem.id), movie_title:curItem.title||curItem.name, movie_poster:curItem.poster_path?`${IMG}w300${curItem.poster_path}`:'', media_type:curType, season:curSeason, episode:curEp, room_code:wpRoomCode, is_active:true },token); } catch(e){} openWatchPartyUI(); initBroadcast(true); }
async function joinWatchParty(code) { if (!currentUser) { pendingRoomCode=code; showLogin(); return; } showJoinSplash('🎉 جاري الانضمام...'); wpRoomCode=code; wpIsHost=false; const token = JSON.parse(localStorage.getItem('shush_session')).access_token; try { const rooms = await gatewayRequest('rooms','GET',{ room_code:code },token); if (rooms && rooms.length>0 && rooms[0].is_active!==false) { const r=rooms[0]; curItem={ id:r.movie_id, title:r.movie_title, poster_path:r.movie_poster?.replace(IMG+'w300',''), media_type:r.media_type }; curType=r.media_type; curSeason=r.season; curEp=r.episode; hideJoinSplash(); openWatchPartyUI(); initBroadcast(false); } else showJoinError('❌ الغرفة غير متاحة'); } catch(e){ showJoinError('❌ فشل الانضمام'); } }
function openWatchPartyUI() { toggleModal('watchPartyModal',true); document.body.style.overflow='hidden'; if($('wpPlayerPlaceholder')) $('wpPlayerPlaceholder').style.display='none'; const iframe = $('wpPlayerFrame'); if(iframe) { iframe.style.display='block'; iframe.src = SRCS[0](curType,curItem.id,curSeason,curEp); iframe.setAttribute('allow','autoplay; encrypted-media; fullscreen; picture-in-picture'); iframe.onload = () => { if (iframe.hasAttribute('sandbox')) iframe.removeAttribute('sandbox'); }; } if($('wpChat')) $('wpChat').innerHTML=''; if($('wpEndBtn')) $('wpEndBtn').style.display = wpIsHost ? 'flex' : 'none'; wpMembers=[]; updateUsersListBroadcast(); if (wpIsHost) window.addEventListener('message',handlePlayerMessage); }
function updateUsersListBroadcast() { const usersList=$('wpUsersList'); if(!usersList) return; usersList.querySelectorAll('.wp-user-item').forEach(el=>el.remove()); const myName=currentUser?.user_metadata?.display_name||'أنا'; const myAvatar = currentUser?.user_metadata?.avatar || '😊'; const myDiv=document.createElement('div'); myDiv.className='wp-user-item'; myDiv.innerHTML=`<div class="user-name"><span class="user-dot"></span><span>${avatarToIcon(myAvatar)}</span><span>${myName}</span>${wpIsHost?'<span class="host-badge">المضيف</span>':''}</div>`; usersList.appendChild(myDiv); wpMembers.forEach(m=>{ if(m.userId!==currentUser?.id){ const userDiv=document.createElement('div'); userDiv.className='wp-user-item'; userDiv.innerHTML=`<div class="user-name"><span class="user-dot"></span><span>${avatarToIcon(m.avatar || '👤')}</span><span>${m.displayName}</span></div>`; usersList.appendChild(userDiv); } }); }
function appendChatMessage(name,msg,isMe){ const chat=$('wpChat'); const div=document.createElement('div'); div.className='wp-chat-msg'; const avatarIcon = isMe ? (currentUser?.user_metadata?.avatar || '😊') : '👤'; div.innerHTML=`<div class="avatar">${avatarToIcon(avatarIcon)}</div><div class="content"><div class="name">${name}</div>${msg}</div>`; chat.appendChild(div); chat.scrollTop=chat.scrollHeight; }
function sendChatMessage(){ const input=$('wpMessageInput'); const msg=input.value.trim(); if(!msg) return; const displayName=currentUser?.user_metadata?.display_name||'مجهول'; appendChatMessage(displayName,msg,true); sendChatMessageBroadcast(msg); input.value=''; }
function closeWatchParty(){ toggleModal('watchPartyModal',false); document.body.style.overflow=''; if($('wpPlayerFrame')) $('wpPlayerFrame').src=''; if(wpChannel){ if(wpIsHost && wpMembers.length) wpChannel.send({type:'broadcast',event:'member_leave',payload:{userId:currentUser.id}}); wpChannel.unsubscribe(); wpChannel=null; } wpRoomCode=null; wpIsHost=false; wpMembers=[]; window.removeEventListener('message',handlePlayerMessage); }
async function endWatchParty(){ if(!wpIsHost) return; if(confirm('إنهاء الغرفة؟')){ try{ const token=JSON.parse(localStorage.getItem('shush_session')).access_token; await gatewayRequest('rooms','PUT',{id:wpRoomCode,is_active:false},token); }catch(e){} showToast('تم إنهاء الغرفة'); closeWatchParty(); } }
function inviteFriendsToRoom(){ if(friends.length===0){ showToast('لا يوجد أصدقاء'); return; } const list=$('inviteFriendsList'); list.innerHTML=friends.map(f=>`<div class="friend-item"><div class="finfo"><div class="favatar">${avatarToIcon(f.avatar||'👤')}</div><span>${f.display_name}</span></div><button class="login-btn" onclick="sendRoomInvite('${f.id}')">دعوة</button></div>`).join(''); toggleModal('inviteFriendsModal',true); }
async function sendRoomInvite(friendId){ try { await dbClient.from('notifications').insert({ user_id:friendId, type:'room_invite', sender_id:currentUser.id, message:'يدعوك لمشاهدة عمل معاً', data:{ roomCode:wpRoomCode } }); showToast('✅ تم إرسال الدعوة'); closeInviteModal(); } catch(e){ console.error(e); showToast('❌ فشل الإرسال'); } }
function showJoinSplash(text){ const splash=$('joinSplash'); if(splash){ if($('joinSplashText')) $('joinSplashText').textContent=text; if($('joinSplashError')) $('joinSplashError').style.display='none'; if($('joinSplashRetry')) $('joinSplashRetry').style.display='none'; splash.classList.add('active'); } }
function showJoinError(msg){ const splash=$('joinSplash'); if(splash){ if($('joinSplashText')) $('joinSplashText').textContent=''; if($('joinSplashError')) { $('joinSplashError').textContent=msg; $('joinSplashError').style.display='block'; } if($('joinSplashRetry')) $('joinSplashRetry').style.display='block'; } }
function hideJoinSplash(){ const splash=$('joinSplash'); if(splash) splash.classList.remove('active'); }
...
function startProgressTimer() { if (progressTimer) clearInterval(progressTimer); progressTimer = setInterval(() => { if (lastSyncTime > 0 && curItem) saveProgressToDB(curItem.id, curType, curSeason, curEp, lastSyncTime, 0); }, 5000); }
function stopProgressTimer() { if (progressTimer) { clearInterval(progressTimer); progressTimer = null; } if (lastSyncTime > 0 && curItem) saveProgressToDB(curItem.id, curType, curSeason, curEp, lastSyncTime, 0); }
...
if($('wpMessageInput')) $('wpMessageInput').addEventListener('keydown', e => { if (e.key === 'Enter') sendChatMessage(); });

(async function() { const splash = document.getElementById('splash-screen'); const minSplashTime = new Promise(resolve => setTimeout(resolve, 700)); try { await Promise.all([init(), checkSession(), minSplashTime]); const code = new URLSearchParams(window.location.search).get('room'); if (code) { if (!currentUser) { pendingRoomCode = code; showLogin(); } else { showJoinSplash('🎉 جاري الانضمام...'); await joinWatchParty(code); } } originalMainHTML = $('main')?.innerHTML || ''; } catch (e) { console.error(e); } finally { if (splash) { splash.classList.add('fade-out'); setTimeout(() => splash.remove(), 300); } } })();
