...
// مصادر الفيديو (غرفة المشاهدة تستخدم الأول فقط VidFast)
const SRCS = [
...
// إنشاء عميل واحد فقط في النطاق العام
const supabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY);

// دالة للحصول على عميل مع Token (للـ RLS)
let dbClient = supabase; // الافتراضي بدون Token
...
function showNotification(notif) {
    ...
    if (notif.type === 'friend_request') {
        ...
        icon = '🎬'; text = `${notif.sender?.display_name || 'صديق'} يدعوك للمشاهدة!`;
        btnText = 'قبول وانضمام'; btnAction = `joinWatchParty('${notif.data?.roomCode}'); this.parentElement.remove();`;
    } else if (notif.type === 'recommendation') {
        icon = '⭐'; text = `${notif.sender?.display_name || 'صديق'} يرشح لك هذا العمل`;
        btnText = 'عرض التفاصيل'; btnAction = `openRecommendedMovie('${notif.data?.movie_id}', '${notif.data?.media_type}'); this.parentElement.remove();`;
    } else if (notif.type === 'direct_message') {
        icon = '💬'; text = `رسالة جديدة من ${notif.sender?.display_name || 'صديق'}`;
        btnText = 'رد على الرسالة'; btnAction = `openDM('${notif.sender_id}', '${notif.sender?.display_name || 'صديق'}'); this.parentElement.remove();`;
    } else return;
...
$('tabLoginModal')?.addEventListener('click', () => {
    $('tabLoginModal').classList.add('active');
    $('tabRegisterModal').classList.remove('active');
    $('loginFormModal').style.display = 'block';
    $('registerFormModal').style.display = 'none';
});
$('tabRegisterModal')?.addEventListener('click', () => {
    $('tabRegisterModal').classList.add('active');
    $('tabLoginModal').classList.remove('active');
...
$('btnLogin')?.addEventListener('click', async () => {
    ...
    try {
        ...
        if (error) {
            ...
            const greetingSplash = $('greetingSplash');
...
async function loadUserData() {
    ...
    try {
        const session = JSON.parse(localStorage.getItem('shush_session'));
        if (!session) return;
        const token = session.access_token;
        dbClient = getAuthClient(token);
        await dbClient.realtime.setAuth(token);
...
async function loadNotifications() {
    ...
    if (!notifs?.length) { allNotifications = []; }
    else {
        ...
        allNotifications = notifs.map(n => ({ ...n, sender: senders?.find(s => s.id === n.sender_id) || null }));
    }
...
function openNotifications() {
    ...
    if (!list) return;
    if (!allNotifications.length) { list.innerHTML = '<p style="text-align:center;color:var(--t3);padding:20px;">لا توجد إشعارات</p>'; return; }
    list.innerHTML = allNotifications.map(n => {
        const sName = n.sender?.display_name || 'مستخدم';
        const sAv = n.sender?.avatar || '👤';
        let actionHtml = '';
...
async function loadDMs(friendId) {
    ...
    if (!data?.length) { list.innerHTML = '<p style="text-align:center;color:var(--t3);padding:20px;">لا توجد رسائل سابقة</p>'; return; }
...
function openRecommendModal() {
    ...
    if (!friends.length) { showToast('يجب إضافة أصدقاء أولاً'); return; }
...
async function searchFriend() {
    ...
    const resultDiv = $('friendSearchResult'); if (!resultDiv) return;
    ...
    if (data?.length) {
        resultDiv.innerHTML = data.filter(u => u.id !== currentUser.id).map(u => {
            ...
            const btnHtml = isFriend ? '<span style="font-size:10px;color:var(--green);background:var(--bg4);padding:4px 8px;border-radius:10px;">صديق لديك</span>' : `<button class="login-btn" style="width:auto;padding:6px 14px;font-size:11px;" onclick="sendFriendRequest('${u.id}')">إضافة ➕</button>`;
            return `<div class="friend-item"><div class="finfo"><div class="favatar">${avatarToIcon(u.avatar || '👤')}</div><span>${u.display_name}</span></div>${btnHtml}</div>`;
        }).join('');
...
async function loadFriendRequests() {
    ...
    if (!requests?.length) { friendRequests = []; }
    else {
        const senderIds = [...new Set(requests.map(r => r.sender_id))];
        const { data: senders } = await dbClient.from('profiles').select('id, display_name, avatar').in('id', senderIds);
        friendRequests = requests.map(req => ({ ...req, sender: senders?.find(s => s.id === req.sender_id) || null }));
    }
    renderFriendRequestsUI();
}
function renderFriendRequestsUI() {
    const list = $('friendRequestsList'); if (!list) return;
    if (!friendRequests.length) { list.innerHTML = '<p style="color:var(--t3);">لا توجد طلبات</p>'; return; }
...
async function loadFriends() {
    ...
    if (!friendRows?.length) { friends = []; }
    else {
        const fIds = friendRows.map(f => f.friend_id);
...
function renderFriendsListUI() {
    ...
    if (!friends.length) { list.innerHTML = '<p style="color:var(--t3);">لا يوجد أصدقاء</p>'; return; }
...
function openSettings() {
    closeUserDropdown();
    if (!currentUser) return;
    if ($('setDisplayName')) $('setDisplayName').value = currentUser.user_metadata?.display_name || '';
    if ($('setNewPass')) $('setNewPass').value = '';
    const currentAvatar = currentUser.user_metadata?.avatar || '😊';
    const avatarSelection = $('avatarSelection');
    if (avatarSelection) {
...
// ========== TMDB & MEDIA RENDERING ==========
function hideAllSections() {
    if ($('sec-all')) $('sec-all').style.display = 'none';
    if ($('sec-fav')) $('sec-fav').style.display = 'none';
    if ($('sec-hist')) $('sec-hist').style.display = 'none';
    if ($('search-results')) $('search-results').style.display = 'none';
}
function showAllSections() {
    if ($('sec-all')) $('sec-all').style.display = 'block';
    if ($('sec-fav')) $('sec-fav').style.display = 'none';
    if ($('sec-hist')) $('sec-hist').style.display = 'none';
    if ($('search-results')) $('search-results').style.display = 'none';
}
...
async function loadHero() {
    try {
        let r = await api('/trending/all/week');
        let items = (r.results || []).filter(x => x.poster_path && x.backdrop_path);
        ...
        $('heroDots').innerHTML = heroItems.map((_, i) => `<button class="hdot${i === 0 ? ' active' : ''}" data-idx="${i}"></button>`).join('');
        document.querySelectorAll('.hdot').forEach(b => b.addEventListener('click', () => goToHero(parseInt(b.dataset.idx))));
        updateHeroInfo(0);
        startHeroTimer();
...
function updateHeroInfo(idx) {
    let item = heroItems[idx];
    if (!item) return;
    heroIdx = idx;
    let isM = item.media_type === 'movie' || item.title;
    if ($('htitle')) $('htitle').textContent = item.title || item.name;
    if ($('hdesc')) $('hdesc').textContent = item.overview || '';
    if ($('hmeta')) $('hmeta').innerHTML = `<span>⭐ ${(item.vote_average || 0).toFixed(1)}</span><span>${(item.release_date || item.first_air_date || '').slice(0, 4)}</span><span style="background:var(--bg4);padding:2px 6px;border-radius:6px">${isM ? 'فيلم' : 'مسلسل'}</span>`;
}
...
async function initGenres() {
    try {
        let r = await api('/genre/movie/list');
        allGenres = r.genres.slice(0, 14);
        if ($('genreBar')) $('genreBar').innerHTML = '<button class="gbtn on" onclick="clearGenre(this)">🔥 الكل</button>' + allGenres.map(g => `<button class="gbtn" onclick="pickGenre(${g.id},'${g.name}',this)">${g.name}</button>`).join('');
    } catch(e) { console.error(e); }
}
function clearGenre(el) {
    ...
    el.classList.add('on');
    showAllSections();
    document.querySelector('#sec-genre-dynamic')?.remove();
}
async function pickGenre(id, name, el) {
    ...
    hideAllSections();
    document.querySelector('#sec-genre-dynamic')?.remove();
    const mt = curTab === 'tv' ? 'tv' : 'movie';
    let r = await api(`/discover/${mt}?with_genres=${id}&page=1`);
    let items = (r.results || []).slice(0, 18);
    ...
    items.forEach(i => sec.querySelector('.grid').appendChild(mkCard(i)));
    $('main').prepend(sec);
}
...
const fetchDetail = async (t, id) => {
    const data = await api(`/${t}/${id}?append_to_response=credits,seasons`);
    try {
        const videoData = await fetch(`${T}/${t}/${id}?append_to_response=videos&api_key=${TMDB_KEY}`).then(r => r.json());
...
async function init() {
    ...
    await initGenres();
    await loadHero();
    try { let m = await fetchMov(1); appendCards('g-mov', m); if ($('b-mov')) $('b-mov').style.display = 'block'; } catch(e) { console.error(e); }
    try { let t = await fetchTV(1); appendCards('g-tv', t); if ($('b-tv')) $('b-tv').style.display = 'block'; } catch(e) { console.error(e); }
}
...
function switchTab(tab, el) {
    ...
    document.querySelector('#sec-genre-dynamic')?.remove();
    document.querySelectorAll('.gbtn').forEach(b => b.classList.remove('on'));
    const a = document.querySelector('.gbtn');
    ...
    if (tab === 'fav') { if ($('sec-fav')) $('sec-fav').style.display = 'block'; renderFavorites(); }
        ...
        if ($('sec-all')) $('sec-all').style.display = 'block';
        let trend = $('g-trend')?.parentElement, mov = $('g-mov')?.parentElement, tv = $('g-tv')?.parentElement;
        if (trend) trend.style.display = 'block';
        if (mov) mov.style.display = 'block';
        if (tv) tv.style.display = 'block';
        if (tab === 'movie') { if (trend) trend.style.display = 'none'; if (tv) tv.style.display = 'none'; }
        else if (tab === 'tv') { if (trend) trend.style.display = 'none'; if (mov) mov.style.display = 'none'; }
    }
}
function renderFavorites() { let g = $('g-fav'); if (!g) return; g.innerHTML = ''; if (!currentFavs.length) { g.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:20px;color:var(--t2)">فارغ</div>'; return; } currentFavs.forEach(f => g.appendChild(mkCard({ id: f.movie_id, title: f.title, poster_path: f.poster ? f.poster.replace(IMG + 'w300', '') : '', media_type: f.type || 'movie', vote_average: 0 }))); }
function renderHistory() { let g = $('g-hist'); if (!g) return; g.innerHTML = ''; if (!currentHistory.length) { g.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:20px;color:var(--t2)">فارغ</div>'; return; } currentHistory.forEach(h => g.appendChild(mkCard({ id: h.movie_id, title: h.title, poster_path: h.poster ? h.poster.replace(IMG + 'w300', '') : '', media_type: h.type || 'movie', vote_average: 0 }))); }
async function toggleFav(item, btn) {
    if (!currentUser) { showLogin(); return; }
    try {
        const token = JSON.parse(localStorage.getItem('shush_session')).access_token;
        const exists = currentFavs.find(f => f.movie_id === item.id);
        if (exists) {
            await gatewayRequest('favorites', 'DELETE', { column: 'id', value: exists.id }, token);
            currentFavs = currentFavs.filter(f => f.id !== exists.id);
            if (btn) { btn.classList.remove('active'); btn.textContent = '🤍'; }
            showToast('تم الإزالة');
        } else {
            const newFav = await gatewayRequest('favorites', 'POST', { user_id: currentUser.id, movie_id: item.id, title: item.title, poster: item.poster || '', type: item.type }, token);
            if (newFav?.length) { currentFavs.push(newFav[0]); if (btn) { btn.classList.add('active'); btn.textContent = '❤️'; } showToast('❤️ أضيف لقائمتي'); }
        }
        if (curTab === 'fav') renderFavorites();
    } catch(e) { console.error(e); }
}
async function addToHistory(item) {
    ...
    try {
        const token = JSON.parse(localStorage.getItem('shush_session')).access_token;
        const existing = currentHistory.find(h => h.movie_id === item.id);
        ...
        const hist = await gatewayRequest('history', 'GET', { columns: '*' }, token);
        currentHistory = hist || [];
        if (curTab === 'hist') renderHistory();
    } catch(e) { console.error(e); }
}
...
async function openDetail(item) {
    ...
    if ($('d-back')) $('d-back').src = ''; if ($('d-poster')) $('d-poster').src = '';
    if ($('dep')) $('dep').style.display = 'none'; if ($('d-seasons')) $('d-seasons').innerHTML = ''; if ($('d-eps')) $('d-eps').innerHTML = '';
    if ($('d-meta')) $('d-meta').innerHTML = ''; if ($('d-cast-wrap')) $('d-cast-wrap').style.display = 'none'; if ($('d-cast')) $('d-cast').innerHTML = '';
    ...
    if (item.backdrop_path && $('d-back')) $('d-back').src = `${IMG}w1280${item.backdrop_path}`;
    if (item.poster_path && $('d-poster')) $('d-poster').src = `${IMG}w500${item.poster_path}`;
    ...
    try {
        ...
        if (!det || (!det.overview && !det.runtime && !det.number_of_seasons)) {
            const altType = curType === 'movie' ? 'tv' : 'movie';
            det = await fetchDetail(altType, item.id);
            if (det && (det.overview || det.runtime || det.number_of_seasons)) curType = altType;
        }
        curItem = { ...item, ...det };
        if (det.backdrop_path && $('d-back')) $('d-back').src = `${IMG}w1280${det.backdrop_path}`;
        if (det.poster_path && $('d-poster')) $('d-poster').src = `${IMG}w500${det.poster_path}`;
        if ($('d-ov')) $('d-ov').textContent = det.overview || item.overview || 'لا يوجد وصف';
        let genresHtml = (det.genres || []).map(g => `<span class="dtag">${g.name}</span>`).join('');
        if ($('d-tags')) $('d-tags').innerHTML += genresHtml;
        let trailer = det.videos?.results?.find(v => v.type === "Trailer" && v.site === "YouTube");
        if (trailer) { trailerKey = trailer.key; if ($('trailerWatchBtn')) $('trailerWatchBtn').classList.add('show'); }
        else { trailerKey = null; if ($('trailerWatchBtn')) $('trailerWatchBtn').classList.remove('show'); }
        let metaHtml = '';
        const addRow = (icon, label, value) => { if (value) metaHtml += `<div class="dmetarow">${icon}<span>${label}</span><span>${value}</span></div>`; };
        addRow('⏱️', 'المدة', det.runtime ? `${Math.floor(det.runtime / 60)}س ${det.runtime % 60}د` : null);
        addRow('🌍', 'البلد', (det.production_countries || []).map(c => c.name).join(', ') || null);
        ...
        if (det.credits?.cast) {
            let cast = det.credits.cast.slice(0, 8);
            if ($('d-cast')) $('d-cast').innerHTML = cast.map(c => `<div class="dcast-item">${c.profile_path ? `<img src="${IMG}w185${c.profile_path}">` : `<div style="width:48px;height:48px;border-radius:50%;background:var(--bg4);display:flex;align-items:center;justify-content:center;font-size:16px">👤</div>`}<span>${c.name}</span></div>`).join('');
            if ($('d-cast-wrap')) $('d-cast-wrap').style.display = 'block';
        }
        if (curType === 'tv' && det.seasons) {
            let seasons = det.seasons.filter(s => s.season_number > 0);
            if (seasons.length) {
                if ($('dep')) $('dep').style.display = 'block';
                if ($('d-seasons')) $('d-seasons').innerHTML = seasons.map((s, i) => `<button class="sbtn ${i === 0 ? 'on' : ''}" onclick="selSeason(${s.season_number},${s.episode_count},this)">${s.name || 'الموسم ' + s.season_number}</button>`).join('');
                renderEps(1, seasons[0]?.episode_count || 0);
            }
        }
    } catch(e) { console.error(e); }
}
...
function performSearch() {
    ...
    hideAllSections();
    if ($('search-results')) $('search-results').style.display = 'block';
    if ($('g-search')) $('g-search').innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:20px;color:var(--t2)">جاري البحث...</div>';
    fetchSearch(q, 1, signal).then(res => {
        if (signal.aborted) return;
        if ($('g-search')) $('g-search').innerHTML = '';
        if (!res.length) { if ($('g-search')) $('g-search').innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:20px;color:var(--t2)">لا توجد نتائج</div>'; }
        else { res.forEach(item => { if (item.media_type !== 'person' && (item.poster_path || item.backdrop_path)) if ($('g-search')) $('g-search').appendChild(mkCard(item)); }); }
    }).catch(() => {});
}
...
function initBroadcast(asHost) {
    ...
    wpChannel.subscribe(async (status) => {
        if (status === 'SUBSCRIBED' && asHost) {
            ...
            setTimeout(() => { if (wpChannel) wpChannel.send({ type: 'broadcast', event: 'request_members', payload: {} }); }, 500);
        }
    });
}
...
async function createWatchParty() {
    ...
    const token = JSON.parse(localStorage.getItem('shush_session')).access_token;
    try {
        await gatewayRequest('rooms', 'POST', {
            host_id: currentUser.id,
            movie_id: String(curItem.id),
            movie_title: curItem.title || curItem.name,
            movie_poster: curItem.poster_path ? `${IMG}w300${curItem.poster_path}` : '',
            media_type: curType,
            season: curSeason,
            episode: curEp,
...
async function joinWatchParty(code) {
    ...
    try {
        ...
        if (rooms?.length && rooms[0].is_active !== false) {
            const r = rooms[0];
            ...
            initBroadcast(false);
            setTimeout(() => {
                if (wpChannel) wpChannel.send({ type: 'broadcast', event: 'member_join', payload: { userId: currentUser.id, displayName: currentUser.user_metadata?.display_name || 'ضيف', avatar: currentUser.user_metadata?.avatar || '👤' } });
            }, 1000);
...
function openWatchPartyUI() {
    ...
    if ($('wpPlayerPlaceholder')) $('wpPlayerPlaceholder').style.display = 'none';
    const iframe = $('wpPlayerFrame');
    if (iframe) {
        iframe.style.display = 'block';
        iframe.src = SRCS[0](curType, curItem.id, curSeason, curEp);
        iframe.setAttribute('allow', 'autoplay; encrypted-media; fullscreen; picture-in-picture');
        iframe.onload = () => { if (iframe.hasAttribute('sandbox')) iframe.removeAttribute('sandbox'); };
    }
    if ($('wpChat')) $('wpChat').innerHTML = '';
    if ($('wpEndBtn')) $('wpEndBtn').style.display = wpIsHost ? 'flex' : 'none';
    wpMembers = [];
    wpMembers.push({ userId: currentUser.id, displayName: currentUser.user_metadata?.display_name || currentUser.email.split('@')[0], avatar: currentUser.user_metadata?.avatar || '😊' });
    updateUsersListBroadcast();
    if (wpIsHost) window.addEventListener('message', handlePlayerMessage);
}
function updateUsersListBroadcast() {
    ...
    for (const m of wpMembers) {
        ...
        const isThisHost = (wpHostId !== null && m.userId === wpHostId);
        const hostBadge = isThisHost ? '<span class="host-badge">المضيف</span>' : '';
        userDiv.innerHTML = `<div class="user-name"><span class="user-dot"></span><span>${avatarToIcon(m.avatar)}</span><span>${m.displayName}</span>${m.userId === currentUser?.id ? ' (أنت)' : ''}${hostBadge}</div>`;
        usersList.appendChild(userDiv);
    }
}
function appendChatMessage(name, msg, isMe) {
    const chat = $('wpChat'); if (!chat) return;
    const div = document.createElement('div'); div.className = 'wp-chat-msg';
    const avatarIcon = isMe ? (currentUser?.user_metadata?.avatar || '😊') : '👤';
    div.innerHTML = `<div class="avatar">${avatarToIcon(avatarIcon)}</div><div class="content"><div class="name">${name}</div>${msg}</div>`;
    chat.appendChild(div); chat.scrollTop = chat.scrollHeight;
}
function sendChatMessage() {
    const input = $('wpMessageInput'); if (!input) return;
...
function closeWatchParty() {
    ...
    const iframe = $('wpPlayerFrame'); if (iframe) iframe.src = '';
    if (wpChannel) {
        if (wpIsHost && wpMembers.length) wpChannel.send({ type: 'broadcast', event: 'member_leave', payload: { userId: currentUser.id } });
    ...
    wpRoomCode = null; wpIsHost = false; wpHostId = null; wpMembers = [];
...
function inviteFriendsToRoom() {
    if (!friends.length) { showToast('لا يوجد أصدقاء'); return; }
    const list = $('inviteFriendsList'); if (!list) return;
    list.innerHTML = friends.map(f => `<div class="friend-item"><div class="finfo"><div class="favatar">${avatarToIcon(f.avatar || '👤')}</div><span>${f.display_name}</span></div><button class="login-btn" onclick="sendRoomInvite('${f.id}')">دعوة</button></div>`).join('');
    toggleModal('inviteFriendsModal', true);
}
async function sendRoomInvite(friendId) {
    try {
        await dbClient.from('notifications').insert({ user_id: friendId, type: 'room_invite', sender_id: currentUser.id, message: 'يدعوك لمشاهدة عمل معاً', data: { roomCode: wpRoomCode } });
...
// ========== MISC & INIT ==========
function dismissClickPrompt() { const promptDiv = document.getElementById('wpClickPrompt'); if (promptDiv) promptDiv.style.display = 'none'; }
...
document.addEventListener('keydown', e => {
    if (e.key === 'Escape') {
        if ($('ppage')?.classList.contains('open')) closePlayer();
        else if ($('dov')?.classList.contains('open')) closeDetail();
        else if ($('trailerModal')?.classList.contains('active')) closeTrailer();
        else if ($('watchPartyModal')?.classList.contains('active')) closeWatchParty();
        else if ($('loginModal')?.classList.contains('active')) closeLoginModal();
        else if ($('settingsModal')?.classList.contains('active')) closeSettings();
        else if ($('friendsModal')?.classList.contains('active')) closeFriends();
        else if ($('adminModal')?.classList.contains('active')) closeAdmin();
        else if ($('recommendModal')?.classList.contains('active')) closeRecommendModal();
        else if ($('dmModal')?.classList.contains('active')) closeDMModal();
        else if ($('notificationsModal')?.classList.contains('active')) closeNotifications();
    }
});
...
(async function() {
    ...
    try {
        ...
        if (roomCode) {
            if (!currentUser) { pendingRoomCode = roomCode; showLogin(); }
            else { showJoinSplash('🎉 جاري الانضمام...'); await joinWatchParty(roomCode); }
        }
